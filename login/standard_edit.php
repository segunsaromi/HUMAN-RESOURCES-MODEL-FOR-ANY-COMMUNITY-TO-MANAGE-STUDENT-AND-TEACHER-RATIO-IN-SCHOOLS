...
while (!$unique_ref_found) {  
    ...
    $query = mysqli_query($con, "SELECT `standard_id` FROM ` standard_tbl` 
              WHERE `standard_id`='".$unique_ref."'");  
...
if (@$_GET['standard_id'] <> ""){
	...
	$school=mysqli_query($conn,"SELECT * FROM standard_tbl WHERE standard_id='$standard_id'");
	$rowschool=mysqli_fetch_array($school);

$standard_name=$rowschool['standard_name'];
$standard_ratio=$rowschool['standard_ratio'];
...
<div style="background-image: url(onlineapp/ccslogo/header.jpg); background-repeat: no-repeat center center; background-size: cover; height: 200px"></div>
	...
	<div class="container">
	  <div class="col-lg-12">
		...
		<div class="row">
		<br>
		<h4 align="center">STANDARD DETAIL FORM</h4><br>
			<div id="form-content">

			<form method="post" id="reg-form" autocomplete="off">
				
				
<input name="standard_id" type="hidden" value="<?php if (@$standard_id == ""){echo $playernumber;} else {echo $standard_id;} ?>">


<div class="form-group">							  <label>Standard Name</label>

					<input type="text" onkeyup="this.value = this.value.toUpperCase();" pattern="^[A-Za-z0-9 -]+$" class="form-control" value="<?php echo @$standard_name; ?>" name="standard_name" id="lname" placeholder="Enter Standard Name" required />
				</div>


<div class="form-group">							  <label>Ratio</label>

					<input type="text" onkeyup="this.value = this.value.toUpperCase();" pattern="^[A-Za-z0-9 -]+$" class="form-control" value="<?php echo @$standard_ratio; ?>" name="standard_ratio" id="lname" placeholder="Enter Student / One Teacher" required />
				</div>
                ...
                <input name="standard" type="checkbox" value="agree" required> I agree to the Terms and Condition
				</div>
